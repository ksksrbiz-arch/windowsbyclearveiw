...
const files = {
  ...
  copilotApi: fs.readFileSync('functions/internal/api/copilot.js', 'utf8'),
...
const checks = [
  ...
  ['lead analyzer output bounded', files.analyzerApi.includes('MAX_OUTPUT_CHARS = 7000') && files.analyzerApi.includes('cleanOutput')],
  ...
  ['command summary output bounded', files.summaryApi.includes('MAX_OUTPUT_CHARS = 7000') && files.summaryApi.includes('cleanText')],
  ['copilot request/output bounds', files.copilotApi.includes('MAX_MESSAGE_LENGTH = 1000') && files.copilotApi.includes('MAX_OUTPUT_TOKENS = 900')],
