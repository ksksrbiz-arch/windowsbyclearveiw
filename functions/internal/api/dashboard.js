  ...
  const [counts, recentLeads, recentQuotes, sources, cities, tasks, recentJobs] = await Promise.all([
    env.QUOTES_DB.prepare(`
      SELECT
        ...
        (SELECT COUNT(*) FROM tasks WHERE status = 'open' AND due_at IS NOT NULL AND date(due_at) = date('now')) AS tasks_today,
        (SELECT COUNT(*) FROM tasks WHERE status = 'open' AND due_at IS NOT NULL AND due_at < datetime('now')) AS tasks_overdue
    `).bind(weekAgo).first(),
    ...
    env.QUOTES_DB.prepare(`
      SELECT t.id, t.title, t.due_at, t.status, t.lead_id, l.name AS lead_name, l.city AS lead_city
    ...
    `).all(),
    env.QUOTES_DB.prepare(`
      SELECT j.id, j.status, j.scheduled_date, j.customer_name, j.customer_city, q.total_cents
