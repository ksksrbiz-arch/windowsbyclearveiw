...
export async function onRequestPatch(context){const db=context.env.QUOTES_DB;await ensureSchema(db);const jobId=new URL(context.request.url).searchParams.get('jobId');if(!jobId)return json({error:'jobId is required.'},400);const r=await readiness(db,jobId);if(r.error)return json({error:r.error},r.status);if(String(r.job.status).toLowerCase()==='cancelled')return json({error:'Cancelled jobs cannot be closed out.',code:'JOB_CANCELLED'},409);let body;try{body=await context.request.json();}catch{return json({error:'Invalid JSON.'},400);}
 ...
 const now=new Date().toISOString();const finalizedAt=finalize?now:null;await db.prepare(`INSERT INTO job_closeouts (job_id,walkthrough_confirmed,warranty_handoff_confirmed,balance_confirmed,signoff_name,signoff_notes,finalized_at,updated_at,updated_by) VALUES (?,?,?,?,?,?,?,?, 'mark') ON CONFLICT(job_id) DO UPDATE SET walkthrough_confirmed=excluded.walkthrough_confirmed,warranty_handoff_confirmed=excluded.warranty_handoff_confirmed,balance_confirmed=excluded.balance_confirmed,signoff_name=excluded.signoff_name,signoff_notes=excluded.signoff_notes,finalized_at=excluded.finalized_at,updated_at=excluded.updated_at,updated_by=excluded.updated_by`).bind(jobId,walkthrough,warranty,balance,name,notes,finalizedAt,now).run();return json({ok:true,closeout:await load(db,jobId),readyForFinalize:r.readyForFinalize});}
