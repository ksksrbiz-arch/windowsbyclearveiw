...
function invoiceIdForQuote(quoteId) { return `INV-${quoteId.replace(/^Q-/, '')}`; }
function invoiceNumberForQuote(quoteId) { return `INV-${quoteId.replace(/^Q-/, '')}`; }
function money(cents) { return `$${(Number(cents || 0) / 100).toLocaleString('en-US', { maximumFractionDigits: 0 })}`; }
function esc(value) {
...
export async function ensureInvoiceForQuote(db, quoteId, { finalize = false } = {}) {
  ...
  const nextStatus = finalize ? 'open' : 'draft';
...
function renderEmail(invoice, items) {
  ...
  const discount = Number(invoice.discount_cents || 0) > 0
    ...
    : '';
  return `<!doctype html><html><body style="margin:0;background:#f3f4f6;padding:28px 12px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td align="center"><table role="presentation" width="640" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:640px;background:#ffffff;border:1px solid #e5e7eb;"><tr><td style="padding:28px 30px;border-bottom:2px solid #111827;font-family:Arial,Helvetica,sans-serif;"><div style="font-size:22px;font-weight:700;color:#111827;">Clearview Windows</div><div style="margin-top:4px;font-size:12px;color:#6b7280;">Clear View Windows &amp; Trim LLC · (564) 208-0801</div></td></tr><tr><td style="padding:26px 30px;font-family:Arial,Helvetica,sans-serif;"><div style="font-size:12px;text-transform:uppercase;letter-spacing:1px;color:#6b7280;">Invoice</div><div style="margin-top:4px;font-size:26px;font-weight:700;color:#111827;">${esc(invoice.invoice_number)}</div><div style="margin-top:6px;font-size:13px;color:#6b7280;">Issued ${new Date(invoice.created_at).toLocaleDateString('en-US')}</div></td></tr><tr><td style="padding:0 30px 24px;font-family:Arial,Helvetica,sans-serif;"><div style="font-size:12px;text-transform:uppercase;letter-spacing:1px;color:#6b7280;">Bill to</div><div style="margin-top:6px;font-size:16px;font-weight:700;color:#111827;">${esc(invoice.customer_name)}</div><div style="margin-top:4px;font-size:13px;color:#4b5563;">${address || 'Address on file'}</div><div style="margin-top:3px;font-size:13px;color:#4b5563;">${esc(invoice.customer_email || '')}</div></td></tr><tr><td style="padding:0 30px 8px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><thead><tr><th align="left" style="padding-bottom:8px;font-family:Arial,Helvetica,sans-serif;font-size:11px;text-transform:uppercase;color:#6b7280;">Description</th><th align="center" style="padding-bottom:8px;font-family:Arial,Helvetica,sans-serif;font-size:11px;text-transform:uppercase;color:#6b7280;">Qty</th><th align="right" style="padding-bottom:8px;font-family:Arial,Helvetica,sans-serif;font-size:11px;text-transform:uppercase;color:#6b7280;">Unit</th><th align="right" style="padding-bottom:8px;font-family:Arial,Helvetica,sans-serif;font-size:11px;text-transform:uppercase;color:#6b7280;">Amount</th></tr></thead><tbody>${itemRows}</tbody></table></td></tr><tr><td style="padding:14px 30px 24px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td colspan="3" style="padding:5px 0;text-align:right;font-family:Arial,Helvetica,sans-serif;color:#6b7280;">Subtotal</td><td style="padding:5px 0;text-align:right;font-family:Arial,Helvetica,sans-serif;color:#111827;">${money(invoice.subtotal_cents)}</td></tr>${discount}<tr><td colspan="3" style="padding:12px 0 0;text-align:right;font-family:Arial,Helvetica,sans-serif;font-size:16px;font-weight:700;color:#111827;border-top:2px solid #111827;">Amount due</td><td style="padding:12px 0 0;text-align:right;font-family:Arial,Helvetica,sans-serif;font-size:22px;font-weight:700;color:#111827;border-top:2px solid #111827;">${money(invoice.total_cents)}</td></tr></table></td></tr><tr><td style="padding:22px 30px;background:#f9fafb;font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:20px;color:#4b5563;">This invoice was generated from your finalized Clearview quote. Payment terms and project terms remain those shown on the signed agreement. Questions? Reply to this email or call (564) 208-0801.</td></tr></table></td></tr></table></body></html>`;
}
...
export async function emailInvoice(context, invoiceId) {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from, to: [invoice.customer_email], reply_to: replyTo, subject: `${invoice.invoice_number} from Clearview Windows`, html: renderEmail(invoice, items) }),
  });
