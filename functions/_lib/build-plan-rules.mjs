export const BUILD_PLAN_KNOWLEDGE_VERSION = '2026-09-08.1';

// This ruleset intentionally separates durable installation principles from
// product-specific requirements. Product-specific fasteners, clearances,
// flashing sequences, sealants, and warranty conditions must be verified
// against the selected manufacturer's written instructions.
export const AUTHORITIES = [
  { id: 'WA-R703', title: 'Washington 2021 IRC adoption / R703 exterior covering', status: 'authoritative', note: 'Washington adopts the 2021 IRC with state amendments; window/door openings are subject to exterior-covering/flashing requirements.' },
  { id: 'FGIA-INSTALL', title: 'FGIA InstallationMasters / manufacturer instructions', status: 'industry-standard', note: 'Installation practice should follow the specific fenestration manufacturer written installation and flashing instructions.' },
  { id: 'BSC-DRAIN', title: 'Building Science Corporation drained-opening principles', status: 'building-science', note: 'Water that reaches an opening should have a controlled path to the exterior; opening drainage is preferred over relying on sealant alone.' },
];

export const MATERIAL_RULES = [
  { id:'M-UNIT', name:'Quoted window/door unit', category:'quote-derived', trigger:'always', action:'Buy exactly the quoted product, size, configuration, handing and quantity; verify against order confirmation.' },
...
export const QC_RULES = [
  'Ordered product/configuration matches the opening',
  'Unit is properly aligned and supported per product instructions',
  'Fasteners and fastening method verified against product instructions',
  'Flashing/water-management sequence completed and visible transitions checked',
  'Sealant is compatible and continuous where required',
  'Interior perimeter air sealing/insulation completed as specified',
  'Drainage/weep paths remain clear',
  'Unit operates, locks and latches correctly',
  'Interior/exterior finish is clean and undamaged',
  'Required installation and closeout photos captured',
];

export function materialPlan({ projectType='unknown', openings=[] } = {}) {
  const types = openings.map(o => o.openingType).filter(Boolean);
  const hasDoor = types.some(t => t === 'sliding door');
  const replacement = projectType !== 'new-construction';
  const materials = MATERIAL_RULES.filter(r => {
    if (r.trigger === 'always') return true;
    if (r.trigger === 'replacement') return replacement;
    if (r.trigger === 'window-or-door') return openings.length > 0;
    if (r.trigger === 'opening-and-installation-path') return openings.length > 0;
    if (r.trigger === 'perimeter-sealing') return openings.length > 0;
    if (r.trigger === 'perimeter-gap') return openings.length > 0;
    if (r.trigger === 'new-construction-or-exposed-opening') return projectType === 'new-construction';
    if (r.trigger === 'finish-scope') return true;
    return false;
  });
  if (!hasDoor) return materials;
  return materials;
}
...
export function lintPlan(plan, items=[]) {
  ...
  const buy = Array.isArray(plan?.buy) ? plan.buy : [];
  const verify = Array.isArray(plan?.verify) ? plan.verify : [];
  const install = Array.isArray(plan?.install) ? plan.install : [];
  const buyText = normalize(buy.join(' '));
  const verifyText = normalize(verify.join(' '));
  const installText = normalize(install.join(' '));
  ...
  if (/(#?\s*\d+\s*x\s*\d+\s*(screw|fastener)|\d+\s*in\.?\s*o\.c\.|on center)/.test(buyText + installText)) blockers.push('The plan contains a specific fastener or spacing specification without a product-data source. Remove it or attach the manufacturer's requirement.');
  ...
  if (items.length && openings.length) {
    const quotedQty = items.reduce((n,i) => n + Math.min(50, Math.max(1, Number(i.quantity)||1)), 0);
    if (quotedQty !== openings.length) warnings.push(`Quote quantity (${quotedQty}) and opening schedule (${openings.length}) do not match; reconcile before ordering.`);
  }
  return { blockers, warnings, knowledgeVersion: BUILD_PLAN_KNOWLEDGE_VERSION };
}

export function sourceSnapshot(items=[]) {
